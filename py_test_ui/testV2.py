                    ...
                    with Progress() as progress:
                        ...
                        for i in range(0, len(df), chunk_size):
                            ...
                            with ThreadPoolExecutor() as executor:
                                # Start processing all rows in the chunk concurrently
                                for idx, row in chunk.iterrows():
                                    ...
                                    if row[SEARCH_KEY[0]] == 'nan':
                                        futures.append((idx, None, None, None, None))
                                        continue
                                    
                                    # Display progress for each row
                                    current_index = i + (idx - chunk.index[0]) + 1
                                    print('[green]{}[/green] / [blue]{}[/blue] [yellow]Processing[/yellow] [light blue]{}[/light blue]'.format(
                                        str(current_index), str(len(df)), row[SEARCH_KEY[0]])
                                    )
                                    logging.info(
                                        'Processing: {}'.format(row[SEARCH_KEY[0]]))
                                    
                                    # Submit all tasks for this row in parallel
                                    pm_future = cal_future = pm_attach_future = cal_attach_future = None
                                    
                                    if option == 'close_pm_cal' or option is None:
                                        pm_future = executor.submit(closePM, row)
                                        cal_future = executor.submit(closeCAL, row)
                                    
                                    if option == 'attach_pm_cal' or option is None:
                                        pm_attach_future = executor.submit(attachFilePM, file_name_list, row)
                                        cal_attach_future = executor.submit(attachFileCAL, file_name_list, row)
                                    
                                    futures.append((idx, pm_future, cal_future, pm_attach_future, cal_attach_future))
                                
                                # Wait for all futures to complete
                                for idx, pm_future, cal_future, pm_attach_future, cal_attach_future in futures:
                                    if idx is None:
                                    ...
                                    # Type conversion for dataframe columns
                                    ...
                                    # Get the ID code for logging
                                    id_code = df.at[idx, SEARCH_KEY[0]]
                                    
                                    # Process PM and CAL closing results
                                    if pm_future:
                                        ...
                                        if process_result_pm.get('status') == 'ok' or process_result_pm.get('status') == 'done':
                                            ...
                                            df.at[idx, 'PM-CLOSED'] = "SUCCESS" if "Completed" in process_result_pm.get('text') else process_result_pm.get('text')
                                            if process_result_pm.get('status') == 'ok':
                                                print('[green]{}[/green] - [blue]{}[/blue]'.format(process_result_pm.get('text'), id_code))
                                                logging.info('PM-CLOSED: {} - ID: {}'.format(process_result_pm.get('text'), id_code))
                                        else:
                                            print('[red]{}[/red] - [blue]{}[/blue]'.format(process_result_pm.get('text'), id_code))
                                            logging.error('PM-CLOSED: {} - ID: {}'.format(process_result_pm.get('text'), id_code))
                                    ...
                                    if cal_future:
                                        ...
                                        if process_result_cal.get('status') == 'ok' or process_result_cal.get('status') == 'done':
                                            ...
                                            df.at[idx, 'CAL-CLOSED'] = "SUCCESS" if "Completed" in process_result_cal.get('text') else process_result_cal.get('text')
                                            if process_result_cal.get('status') == 'ok':
                                                print('[green]{}[/green] - [blue]{}[/blue]'.format(process_result_cal.get('text'), id_code))
                                                logging.info('CAL-CLOSED: {} - ID: {}'.format(process_result_cal.get('text'), id_code))
                                        elif process_result_cal.get('status') != 'done':
                                            print('[red]{}[/red] - [blue]{}[/blue]'.format(process_result_cal.get('text'), id_code))
                                            logging.error('CAL-CLOSED: {} - ID: {}'.format(process_result_cal.get('text'), id_code))
                                            df.at[idx, 'CAL-CLOSED'] = process_result_cal.get('text')
                                    
                                    # Process PM and CAL attachment results
                                    if pm_attach_future:
                                        ...
                                        if process_result_pm_attach.get('status') == 'ok' or process_result_pm_attach.get('status') == 'done':
                                            issave = False if issave == False and process_result_pm_attach.get('nosave') == True else True
                                            df.at[idx, 'PM-ATTACH-STATUS'] = "SUCCESS" if process_result_pm_attach.get('text').startswith('Attach') else process_result_pm_attach.get('text')
                                            if process_result_pm_attach.get('status') == 'ok':
                                                print('[green]{}[/green] - [blue]{}[/blue]'.format(process_result_pm_attach.get('text'), id_code))
                                                logging.info('PM-ATTACH-STATUS: {} - ID: {}'.format(process_result_pm_attach.get('text'), id_code))
                                        else:
                                            print('[red]{}[/red] - [blue]{}[/blue]'.format(process_result_pm_attach.get('text'), id_code))
                                            logging.error('PM-ATTACH-STATUS: {} - ID: {}'.format(process_result_pm_attach.get('text'), id_code))
                                    ...
                                    if cal_attach_future:
                                        ...
                                        if process_result_cal_attach.get('status') == 'ok' or process_result_cal_attach.get('status') == 'done':
                                            issave = False if issave == False and process_result_cal_attach.get('nosave') == True else True
                                            df.at[idx, 'CAL-ATTACH-STATUS'] = "SUCCESS" if process_result_cal_attach.get('text').startswith('Attach') else process_result_cal_attach.get('text')
                                            if process_result_cal_attach.get('status') == 'ok':
                                                print('[green]{}[/green] - [blue]{}[/blue]'.format(process_result_cal_attach.get('text'), id_code))
                                                logging.info('CAL-ATTACH-STATUS: {} - ID: {}'.format(process_result_cal_attach.get('text'), id_code))
                                        else:
                                            print('[red]{}[/red] - [blue]{}[/blue]'.format(process_result_cal_attach.get('text'), id_code))
                                            logging.error('CAL-ATTACH-STATUS: {} - ID: {}'.format(process_result_cal_attach.get('text'), id_code))
                                    ...
                                    if option == 'close_pm_cal':
                                        df.at[idx, 'PM-ATTACH-STATUS'] = row['PM-ATTACH-STATUS'] if row['PM-ATTACH-STATUS'] != 'nan' else ''
                                        df.at[idx, 'CAL-ATTACH-STATUS'] = row['CAL-ATTACH-STATUS'] if row['CAL-ATTACH-STATUS'] != 'nan' else ''
                                    elif option == 'attach_pm_cal':
                                        df.at[idx, 'PM-CLOSED'] = row['PM-CLOSED'] if row['PM-CLOSED'] != 'nan' else ''
                                        df.at[idx, 'CAL-CLOSED'] = row['CAL-CLOSED'] if row['CAL-CLOSED'] != 'nan' else ''
                            ...
                            # Use explicit function to avoid "v is not accessed" warning
                            def format_as_text(value):
                                return "number-format: @"
                            df.style.format(format_as_text).to_excel(
