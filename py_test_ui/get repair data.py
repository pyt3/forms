...
def set_login():
    ...
    global cookies
    try:
        r = session.post("https://nsmart.nhealth-asia.com/MTDPDB01/index.php",
                data=data,
                headers=headers,
                verify=False)
        confdata["SESSION_ID"] = session.cookies['PHPSESSID']
...
async def fetch(session, url):
    async with session.get(url, headers=headers, cookies=cookies) as response:
        # set encoding to tis-620
        text = await response.read()
        decode_text = text.decode('tis-620', 'ignore')
        res = re.findall(r'<table\s+[^>]*class=["\']Grid["\'][^>]*>', decode_text, re.DOTALL)
        if len(res) == 0:
            print(response)
            print("No table found, re-login...")
            set_login()
        ...
        return decode_text
async def get_repairWorks_file(url='https://nsmart.nhealth-asia.com/MTDPDB01/jobs/BJOBA_01_show.php?s_code=&s_code_equip=&s_sap_code=&s_jobno=&s_groupid=&s_catagory=&s_branchid=&s_dept=&s_sub_dept=&s_brand=&s_model=&s_serial_no=&s_date=01%2F01%2F2013&s_to_date=31%2F12%2F2013&s_job_status=&s_jobtype=&s_urgentstat=&jobsPageSize=100', page='0'):
    time_start = time.time()
    global repairWorks_arr
    global max_data
    if page == '1000':
        repairWorks_arr = []
    page_urls = []
    end = int(page)+5 if int(page)+5 <= max_data and max_data is not None else 5
    print(f'Fetching page {(end-4)} to {end} of {(max_data if max_data > 0 else 100000)}')
    for i in range(int(page)+1, end):
        page_urls.append(url + "&jobsPage=" + str(i))
    try:
        ...
        #     verify=False,
        # )
        responses = await fetch_multiple(page_urls)
    ...
    for response in responses:
        ...
        # find table with regex    
        res = re.findall(r'<table\s+[^>]*class=["\']Grid["\'][^>]*>', response, re.DOTALL)
        if len(res) == 0:
            print(response)
        ...
        # index = response.text.find('<table class="Grid" cellspacing="0" cellpadding="0">')
        if max_data <= 0:
            max_data = response.split('Total : ')[1].split(' ')[0].replace(',', '')
            try:
                max_data = int(max_data)
            except:
                max_data = 100000
        tmp = response.split('<table class="Grid"')
        tmp2 = tmp[1].split('</table>')
        ...
        for row in rows:
            cols = row.find_all('td')
            cols = [ele.text.strip() if ele.text.strip() != 'Click' else '' for ele in cols]
            repairWorks_arr.append(cols)
    print('[yellow]Total data: [/yellow][blue]{}[/blue]'.format(len(repairWorks_arr)))
    time.sleep(10)
        
    if len(repairWorks_arr) >= (max_data if max_data > 0 else 100000):
    # if int(page) == 2:
        df = pd.DataFrame(repairWorks_arr, columns=header)
        df.to_excel(os.path.join(root_dir,'EXCEL FILE', 'repair_works_list.xlsx'), index=False)
        # print file path to let user to click
        print('[yellow]The equipment list file has been saved at[/yellow] [blue]{}[/blue]' .format(os.path.join(root_dir, 'EXCEL FILE', 'repair_works_list.xlsx')))
        # open file
        file_path = os.path.join(root_dir, 'EXCEL FILE', 'repair_works_list.xlsx')
        auto_adjust_column_width_from_df(file_path, 'Sheet1')
        os.system('start excel.exe "' + file_path+ '"')
...
try:
    ...
    page = loop.run_until_complete(get_repairWorks_file())
    while page is not None:
        page = loop.run_until_complete(get_repairWorks_file(page=page))
    # loop.run_until_complete(showmenu())
    # page = get_repairWorks_file()
    # while page is not None:
    #     page = get_repairWorks_file(page=page)
except Exception as e:
    print(f"An error occurred on line {sys.exc_info()[-1].tb_lineno}: {e}")
# change_file_name()
# get_emp_list()
