async function getRefrigeratorTableData(session) {
...
function createTemperatureTable(data) {
    $('#refrigerator .table-responsive').html('').append(`<table id="temperature-table-date" class="table table-hover table-bordered" style="width: 100%">
...
function renderTemperatureStatus(value) {
    ...
    let toHight = `<br><span class="badge rounded-pill bg-warning status-pill-modal"><i class="bi bi-exclamation-triangle"></i>&nbsp;อุณหภูมิสูงเกินไป</span>`
    let toLow = `<br><span class="badge rounded-pill bg-danger status-pill-modal"><i class="bi bi-exclamation-triangle"></i>&nbsp;อุณหภูมิต่ำเกินไป</span>`
    switch (true) {
...
async function updateRefrigeratorData(key, url, time, date = new Date()) {
    console.log("🚀 ~ key", key)
