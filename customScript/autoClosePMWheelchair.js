class FormAutomation {
    ...
    removeStage() {
        ...
        localStorage.removeItem('current_wheelchair_data');
    }

    executeStage0() {
        console.log('Stage 0');
        this.setStage('1');
        localStorage.setItem('current_wheelchair_data', JSON.stringify({ date: this.date, pressure: this.pressure, job_id: this.job_id }));
    ...
    executeStage1() {
        ...
        const fillData = [
            ...
            { name: 'app_issue_name', value: '577199' }
        ];
...
setTimeout(() => {
    let currentData = localStorage.getItem('current_wheelchair_data');
    if (currentData) {
        ...
        let dataToRun = localStorage.getItem('wheelchair_data');
        if (dataToRun) {
            dataToRun = JSON.parse(dataToRun.replace(/'/g, '"'));
            if (dataToRun?.length > 0) {
                const nextData = dataToRun.shift();
                localStorage.setItem('wheelchair_data', JSON.stringify(dataToRun));
                localStorage.setItem('current_wheelchair_data', JSON.stringify({ date: nextData.date, pressure: nextData.pressure, job_id: nextData.job_id, code: nextData.code }));
                localStorage.setItem('wheelchair_data_remaining', dataToRun.length);
